...
def create_pie_chart(textblob_results, vader_results):
    """ Create and display pie charts comparing TextBlob and VADER results with counts """
    ...
    # Create labels with both the sentiment and the count for TextBlob
    textblob_labels = [f'{index} ({count})' for index, count in zip(textblob_counts.index, textblob_counts)]

    # Create labels with both the sentiment and the count for VADER
    vader_labels = [f'{index} ({count})' for index, count in zip(vader_counts.index, vader_counts)]

    # Plot pie chart for TextBlob results
    st.write("### TextBlob Sentiment Distribution")
    fig1, ax1 = plt.subplots()
    ax1.pie(textblob_counts, labels=textblob_labels, autopct='%1.2f%%', startangle=140, colors=['#DC381F', '#01F9C6', '#2B65EC'])
    ...
    ax2.pie(vader_counts, labels=vader_labels, autopct='%1.2f%%', startangle=140, colors=['#DC381F', '#01F9C6', '#2B65EC'])
